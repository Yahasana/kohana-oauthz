...
abstract class Oauth_Core {
    ...
    public static function parse_post($post)
    {
        ...
        if (! empty($post))
        {
            if(isset($headers['Content-Type'])
                AND stripos($headers['Content-Type'], 'application/x-www-form-urlencoded') !== FALSE)
            {
                //
            }
        }
    ...
    {
        if(Oauth::$headers !== NULL) return Oauth::$headers;
     ...
     * @return  array
     */
    public static function parse_header($headers, $oauth_only = TRUE)
    {
        $pattern = '/(([-_a-z]*)=("([^"]*)"|([^,]*)),?)/';
        $offset = 0;
        $params = array();
        ...
        while (preg_match($pattern, $headers, $matches, PREG_OFFSET_CAPTURE, $offset) > 0)
        {
            ...
            if (preg_match('/^oauth_/', $header_name) || !$oauth_only)
            {
